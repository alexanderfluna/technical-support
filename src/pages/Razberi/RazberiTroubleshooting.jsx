        ...
        <div className="faq-list">
            <div id="server-power" className="faq-answer">
                ...
                {isFAQExpanded("server-power") && (
                    <>
                        <p><Checkbox/> Document if the server boots up at all or how frequently it powers on and off.</p>
                        <p><Checkbox/> Verify that the AC outlet is producing 100 - 120VAC.</p>
                        <p><Checkbox/> If a UPS is in use, bypass it temporarily to test with direct wall power.</p>
                        <p><Checkbox/> Replace the power cord with another known-working one.</p>
                        <p><Checkbox/> Check for any error messages that may indicate why the server is powering off.</p>
                        <p style={{paddingLeft: "60px"}}><Checkbox/> Check for error messages in Event Viewer.</p>
                        <p style={{paddingLeft: "60px"}}><Checkbox/> Check for error messages in Razberi Monitor.</p>
                        <p><Checkbox/> On an <strong>SS32X server</strong>, confirm the following settings in BIOS.</p>
                        <p style={{paddingLeft: "60px"}}><Checkbox/> In "Advanced" -- "ACPI Settings", make sure "Enable Hibernation" is disabled.</p>
                        <p style={{paddingLeft: "60px"}}><Checkbox/> In "Advanced" -- "ACPI Settings", make sure "ACPI Sleep State" is set to "Suspend Disabled"</p>
                        <p style={{paddingLeft: "60px"}}><Checkbox/> In "Chipset" -- "Board Configuration", make sure "PWR-On After PWR-Fail" is set to "Last state" or "On".</p>
                        <p style={{paddingLeft: "60px"}}><Checkbox/> In "Chipset" -- "PCH-IO Configuration" -- "SATA and RST Configuration", make sure all the hard drives are detected. There should be 4 of them (but it will show up as 0, 1, 2, and 3).</p>
                        <p style={{paddingLeft: "60px"}}><Checkbox/> Check that the boot order is correct. Windows Boot Manager should be option #1.</p>
                        <p><Checkbox/> On a <strong>Core server</strong>, enter System Setup when booting into the server.</p>
                        <p style={{paddingLeft: "60px"}}><Checkbox/> Change the <strong>Thermal Profile</strong> to <strong>Minimum Power (Performance per Watt Optimized).</strong></p>
                        <p style={{paddingLeft: "60px"}}><Checkbox/> The power of the server can be capped by specifing the <strong>watts</strong>, <strong>BTU/hr</strong>, and by a <strong>percentage</strong>. A lower power consumption typically means lower heat output, which can reduce cooling requirements. The fans may run at a lower speed, making the server quieter. Lower power usage can also reduce the thermal stress, potentially extending the life of components like CPUs and power supplies. </p>
                        <p><Checkbox/> If all other options are exhausted, perform a recovery of the OS.</p>
                        <p><Checkbox/> If the issue still persists, there may be an issue with the hardware. Please contact technical support for further assistance.</p>
                    </>
                )}
            </div>
            ...
            <div id="no-poe" className="faq-answer">
                ...
                {isFAQExpanded("no-poe") && (
                    <>
                        <p><Checkbox/> Verify that the AC outlet is producing 100 - 120VAC.</p>
                        <p><Checkbox/> If a UPS is in use, bypass it temporarily to test with direct wall power.</p>
                        <p><Checkbox/> Replace the power cord with a known-working one.</p>
                        <p><Checkbox/> Confirm that the powered device is IEEE 802.3af/at compliant by looking up its data sheet.</p>
                        <p><Checkbox/> Verify that the switch is not producing more PoE than its PoE budget allows.</p>
                        <p><Checkbox/> Try pinging the switch from the server's command prompt.</p>
                        <p><Checkbox/> If the switch is accessible, disable and enable PoE on the port in the switch's web interface.</p>
                        <p><Checkbox/> If the switch is still not providing PoE, default the switch by placing a jumper between ports 1 and 2.</p>
                        <p><Checkbox/> If the issue still persists, there may be an issue with the hardware. Please contact technical support for further assistance.</p>
                    </>
                )}
            </div>
            ...
            <div id="raid" className="faq-answer">
                ...
                {isFAQExpanded("raid") && (
                    <>
                        <p><Checkbox/> Take note of the following RAID configurations.</p>
                        ...
                        <p><Checkbox/> Using one of the following, confirm the server's current RAID configuration and which HDD has failed. </p>
                        ...
                        <p><Checkbox/> Contact technical support to check the server's warranty status. Covered drives receive free replacements. Out-of-warranty drives must be purchased separately.</p>
                        <p><Checkbox/> If the RAID is corrupted, delete the virtual disk, create a new virtual disk with each HDD selected, and format the volume as NTFS using one the following. </p>
                        <li style={{paddingLeft: "60px"}}>BIOS</li>
            ...
            <div id="ssd" className="faq-answer">
                ...
                {isFAQExpanded("ssd") && (
                    <>
                        ...
                        <p><Checkbox/> Ship the server to us, and we can re-image the unit.</p>
                        <p><Checkbox/> We can ship out a new mSATA with our image on it.</p>
                        <p><Checkbox/> Purchase a new mSATA, install the OS, and we can provide the necessary drivers and software.</p>
                    </>
                )}
            </div>
            ...
            <div id="nic" className="faq-answer">
                ...
                {isFAQExpanded("nic") && (
                    <>
                        <p><Checkbox/> Document if (1) the LEDs illuminate on the NIC when an Ethernet connection is made and (2) whether the NIC can be successfully pinged.</p>
                        <p><Checkbox/> Enter "View network connections" in the Windows search box.</p>
                        <p><Checkbox/> In Network Connections, disable and reenable the NIC.</p>
                        <p><Checkbox/> Select the 'Diagnose this connection' option for the NIC to diagnose the issue.</p>
                        <p><Checkbox/> In Device Manager, verify what drivers the NIC has. Contact technical support to determine if all of the necessary drivers are installed for the NIC.</p>
                    </>
                )}
            </div>
            ...
            <div id="os" className="faq-answer">
                ...
                {isFAQExpanded("os") && (
                    <>
                        <p><Checkbox/> Determine the OS image installation date. In PowerShell, type the command <strong>Get-CimInstance -ClassName Win32_OperatingSystem | Select-Object InstallDate</strong>. Make sure this matches what is in Razberi Monitor Cloud. </p>
                        <p><Checkbox/> Open Disk Management to confirm there is only one OS partition and that the RAID is healthy.</p>
                        <p><Checkbox/> Review the server's alert logs to identify the cause of the OS issue.</p>
                        <p style={{paddingLeft: "60px"}}><Checkbox/> Check the Razberi Monitor alert logs.</p>
                        <p style={{paddingLeft: "60px"}}><Checkbox/> Check the Event Viewer alert logs.</p>
                        <p style={{paddingLeft: "60px"}}><Checkbox/> Check the iDRAC alert logs.</p>
                        <p><Checkbox/> Open Task Manager and review the the CPU and memory utilization. If either are high, determine what processes are using the most resources and what services are being used by those processes.</p>
                        <p><Checkbox/> If the OS is not accessible, or if all other options are exhausted, perform a recovery of the OS. Please note it is recommended to back up all of the data on the server as all of the storage will be lost.</p>
                        <p><Checkbox/> If the OS is still experiencing issues after a recovery, contact technical support.</p>
                    </>
                )}
            </div>
            ...
            <div id="rdp" className="faq-answer">
                ...
                {isFAQExpanded("rdp") && (
                    <>
                        <p><Checkbox/> Enable Remote Desktop in the server's settings.</p>
                        <p><Checkbox/> Connect a PC to a NIC on the server.</p>
                        <p><Checkbox/> Open the Remote Desktop Connection software on the PC.</p>
                        <p><Checkbox/> Enter the IP address of the server's NIC.</p>
                        <p><Checkbox/> Enter the server's Windows username and password.</p>
                    </>
                )}
            </div>
            ...
            <div id="camera-defense" className="faq-answer">
                ...
                {isFAQExpanded("camera-defense") && (
                    <>
                        <p><strong>Set up Device Binding</strong></p>
                        ...
                        <p><strong>Set up the Device Groups</strong></p>
                        ...
                        <p><strong>Set up the Firewall</strong></p>
                        ...
                        <p><strong>Set up the Internet Protection</strong></p>
                        ...
                        <p><strong>Set up the Password Protection</strong></p>
